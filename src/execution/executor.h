    ...
    static uintptr_t FindUnloadedModule(HANDLE hProcess, std::string& outName) {
        ...
        auto trySearchFolder = [&](uintptr_t folder) {
            ...
            for (uintptr_t child : rblx::GetChildren(hProcess, folder)) {
                ...
                // CRITICAL anti-detection: only hijack test/story modules that are
                // never required by the game. Touching active UI modules crashes the game.
                std::string mName = rblx::ReadInstanceName(hProcess, child);
                std::string mLower = mName;
                std::transform(mLower.begin(), mLower.end(), mLower.begin(), ::tolower);
                bool isSafeModule = mLower.find("jest")  != std::string::npos ||
                                    mLower.find("test")  != std::string::npos ||
                                    mLower.find("spec")  != std::string::npos ||
                                    mLower.find("story") != std::string::npos;
                if (!isSafeModule) continue;
